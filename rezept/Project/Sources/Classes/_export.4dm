...
Function 診療行為($code : Text; $collection : Collection)
	...
	$c診療行為:=Storage:C1525.診療行為.診療行為.query("診療行為コード === :1"; $code)
	
	If ($c診療行為.length=1)
		var $診療行為 : Object
		$診療行為:=$c診療行為[0]
		Use ($collection)
			$collection.push(OB Copy:C1225($診療行為))
		End use 
	End if 
	
Function コメント($code : Text; $collection : Collection)->$isComment : Boolean
	
	$コメント:=Storage:C1525.コメント.code[$code]
	
	If ($コメント#Null:C1517)
		Use ($collection)
			$collection.push(OB Copy:C1225($コメント))
		End use 
		$isComment:=True:C214
	Else 
		TRACE:C157
...
Function setup_k()
	...
	If ($noHost)
		...
		var $es : cs:C1710._診療行為Selection
		$es:=ds:C1482._診療行為.query("基本漢字名称 in :1"; $names)
		
		var $算定回数が複数月に１回のみとされている検査 : Collection
		$算定回数が複数月に１回のみとされている検査:=$es.extract("基本漢字名称")
					...
					For each ($row; $sheets[0].rows; 1)
						...
						If ($values.join()#"")
							...
							If ($index=1)
								...
								Case of 
									: ($診療行為名称等="悪性腫瘍組織検査１")
										$c診療行為:=ds:C1482._診療行為.query("診療行為コード in :1"; $悪性腫瘍組織検査１)
										
									: ($診療行為名称等="悪性腫瘍遺伝子検査")
										$c診療行為:=ds:C1482._診療行為.query("診療行為コード in :1"; $悪性腫瘍遺伝子検査)
										
									: ($診療行為名称等="障害者施設等入院基本料")
										$c診療行為:=ds:C1482._診療行為.query("診療行為コード in :1"; $障害者施設等入院基本料)
										
									: ($診療行為名称等="一般病棟入院基本料")
										$c診療行為:=ds:C1482._診療行為.query("診療行為コード in :1"; $一般病棟入院基本料)
										
									: ($診療行為名称等="精神病棟入院基本料")
										$c診療行為:=ds:C1482._診療行為.query("診療行為コード in :1"; $精神病棟入院基本料)
										
									: ($診療行為名称等="特定機能病院入院基本料")
										$c診療行為:=ds:C1482._診療行為.query("診療行為コード in :1"; $特定機能病院入院基本料)
										
									: ($診療行為名称等="病理診断料")
										$c診療行為:=ds:C1482._診療行為.query("基本漢字名称 == :1 or 基本漢字名称 == :2"; "組織診断料@"; "細胞診断料@")
										
									: ($診療行為名称等="特定保険医療材料")
										
										$c特定器材:=ds:C1482._特定器材.query("項目.告示番号.区分番号 >= :1 and 項目.告示番号.区分番号 <= :2"; "1"; "209")
										$c診療行為:=$c特定器材.extract("特定器材コード"; "診療行為コード")
										
									: ($診療行為名称等="算定回数が複数月に１回のみとされている検査")
										$c診療行為:=ds:C1482._診療行為.query("基本漢字名称 in :1"; $算定回数が複数月に１回のみとされている検査)
										
									: ($診療行為名称等="ＭＥＴｅｘ14遺伝子検査")
										$c診療行為:=ds:C1482._診療行為.query("基本漢字名称 == :1 or 基本漢字名称 == :2"; "悪性腫瘍組織検査（処理が容易）（医薬品適応判定の補助等）（肺癌におけるＭＥＴｅｘ１４遺伝子検査（次世代シーケンシングを除く。））"; "悪性腫瘍組織検査（処理が複雑なもの）（肺癌におけるＭＥＴｅｘ１４遺伝子検査（次世代シーケンシング））")
										
									: ($診療行為名称等="在宅患者診療・指導料")
										$c診療行為:=ds:C1482._診療行為.query("基本漢字名称 == :1 or 基本漢字名称 == :2"; "在宅患者訪問診療料@"; "在宅患者訪問看護・指導料@")
									Else 
										$c診療行為:=ds:C1482._診療行為.query("基本漢字名称 == :1"; $q)
								End case 
...
*/
		...
		Else 
			...
			If ($file#Null:C1517) && ($file.exists)
				...
				This:C1470[$dataClassName].診療行為コード:=$object.診療行為コード
